...
@section('content')
<div class="container my-4 job-page">
  {{-- Scoped CSS: tidak merubah gaya global selain area .job-page --}}
  <style>
    /* ----- Theme (scoped) ----- */
    .job-page { background-color: transparent; } /* layout wrapper only */
    .job-page .page {
      background-color: #0f1115; color: #e6eef8; padding: 1rem; border-radius: .5rem;
    }
    .job-page a { color: #9ecbff; text-decoration: none; }
    .job-page a:hover { color: #cfe6ff; text-decoration: underline; }
    ...
    .related-list a .small-muted { color:#9aa0ac; }
    ...
    .meta-list li strong { color: #e6eef8; }
    .breadcrumb { margin-bottom: 1rem; color: #9aa0ac; }
    .job-title { color:#f3f7ff; }
    .section-heading { color:#cfe6ff; }

    /* CTA fixed bottom wrapper (pointer-events preserved on buttons) */
    .fixed-cta { left:0; right:0; bottom:18px; z-index:1500; pointer-events:none; position:fixed; width:100%; }
    .fixed-cta .btn { pointer-events:auto; }

    /* Prevent container overlap with fixed CTA */
    .job-page .tw-card { padding-bottom: 4.5rem; }

    @media (max-width: 576px) {
      .tw-card { padding: 1rem; }
      .fixed-cta { bottom:12px; }
    }
  </style>

  <div class="page">
    {{-- Breadcrumb --}}
    <div class="mb-3 breadcrumb small muted">
    ...
    <div class="tw-card">
      {{-- Judul --}}
      <h1 class="h4 fw-bold mb-2 job-title">{{ $job->title ?? 'Tanpa Judul' }}</h1>

      {{-- Rincian Lowongan --}}
      <div class="tw-section mb-4">
        <h2 class="h5 fw-bold mb-3 section-heading">Rincian Lowongan</h2>
        <ul class="list-unstyled small meta-list">
          @if($job->date_posted)
      ...
      <div class="mt-3">
        @auth
          <button class="btn btn-outline-danger" data-bs-toggle="modal" data-bs-target="#reportJobModal">Laporkan Lowongan</button>
        @else
          <a href="{{ route('login') }}" class="btn btn-outline-danger">Login untuk Laporkan</a>
        @endauth
      </div>

      {{-- Jika apply_url kosong tunjukkan alert --}}
      @if(empty($job->apply_url))
        <div class="tw-alert mt-4">Link lamaran eksternal tidak disertakan — gunakan tombol "Lamar Sekarang" di bawah jika tersedia.</div>
      @endif

      {{-- Related jobs --}}
      <div class="tw-section mt-4">
        <h3 class="h6 fw-bold mb-2 section-heading">Loker terbaru</h3>
        @if(!empty($relatedJobs) && $relatedJobs->count() > 0)
          <div class="related-list small">
            @foreach($relatedJobs as $r)
              <a href="{{ url('/loker/'.$r->id) }}" title="{{ $r->title ?? 'Lowongan' }}">
                • {{ $r->title ?? 'Tanpa Judul' }}
                @if($r->date_posted)
      ...
      {{-- 5 Kota Teratas --}}
      <div class="tw-section mt-4">
        <h3 class="h6 fw-bold mb-2 section-heading">5 Kota Teratas — Jelajahi berdasarkan kota</h3>
...
{{-- Auto-open report modal jika validasi gagal --}}
...
{{-- =============================== --}}
{{-- INTERNAL APPLY — CTA + MODAL (tidak diubah fungsionalnya) --}}
{{-- =============================== --}}
@php
...
@if($directApply)
  <div class="fixed-cta d-flex justify-content-center">
    @guest
      <a href="{{ route('login') }}" class="btn btn-primary btn-lg shadow">Login untuk Lamar</a>
    @else
      @if($existingApp)
        <button class="btn btn-secondary btn-lg shadow" disabled>Sudah melamar — {{ ucfirst($existingApp->status ?? 'submitted') }}</button>
      @else
        <button class="btn btn-primary btn-lg shadow" data-bs-toggle="modal" data-bs-target="#applyModal">Lamar Sekarang</button>
      @endif
    @endguest
  </div>
        ...
        <form action="{{ url('/loker/'.$job->id.'/apply') }}" method="POST" enctype="multipart/form-data" class="needs-validation" novalidate>
          ...
          <div class="modal-body">
            ...
            <div class="mb-3">
              ...
              <textarea id="cover_letter" name="cover_letter" rows="4" class="form-control bg-transparent text-light border-secondary">{{ old('cover_letter') }}</textarea>
            </div>

            <div class="mb-3">
              <label for="resume" class="form-label small-muted">Unggah CV (PDF / DOC / DOCX) — maks 5MB (opsional)</label>
              <input id="resume" name="resume" type="file" accept=".pdf,.doc,.docx" class="form-control bg-transparent text-light border-secondary">
  ...
  {{-- Auto-open apply modal on validation errors --}}
  @if($errors->any() && session()->hasOldInput())
    <script>
      document.addEventListener("DOMContentLoaded", function(){
